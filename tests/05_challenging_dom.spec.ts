...
test.describe('Challenging DOM', () => {
    ...
    const readExpected = (): string[][] => {
        ...
        return fileContent.replace(/\r/g, '').trim().split('\n').map(row => row.split(','));
    }

    const readNumberFromDOM = async (page: Page): Promise<string> => {
        const scripts: Locator[] = await page.locator('script').all();
        for (const script of scripts) {
            const scriptContent: string  = await script.evaluate((element) => element.innerHTML);
            if (scriptContent.includes('Answer: ')) {
                return scriptContent.split('Answer: ')[1].split("'")[0].replace(',','');
            }
        }
    ...
    test('Buttons should have random IDs', async ({ page }) => {
       const buttons: Locator[] = await page.locator('//a[contains(@class, "button")]').all();
    ...
    test('Table should have correct data', async ({ page }) => {
        ...
        for (const row of rows) {
            ...
            for (const cell of cells) {
                ...
                const cellContentTrimmed: string | undefined = cellContent?.replace(/\s+/g, '');
                cellData.push(cellContentTrimmed || '');
            }
            tableData.push(cellData);
        }
        expect(tableData).toEqual(readExpected());
    });

    test('Image displays correct number', async ({ page }) => {
        const canvas: ElementHandle<SVGElement | HTMLElement> | null = await page.locator('//canvas').elementHandle()
        if (canvas) {
            ...
            expect(numberFromImage).toEqual(await readNumberFromDOM(page));
        } else {
